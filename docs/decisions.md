...
- **Node is part of the install (revised):** if Node ≥ 20 isn't present, the installer downloads an **official Node build into a user cache and verifies its SHA‑256** (no root) and uses it locally; `--with-deps` remains the opt‑in for a system‑wide Node. The earlier behaviour (bail out unless `--with-deps`) failed for users without Node, so provisioning is now the default — still least‑privilege.
